...
2. Before any non-trivial implementation, consult `architect` for design/consistency review, and get sign-off before delegating the build to `developer`.
...
6. If sub-agents disagree (e.g. architect flags something developer built), resolve it yourself or bring the user a clear decision, not raw disagreement.
