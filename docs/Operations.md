...
The Azure deployment mounts no file share (no SMB, no NFS): the node's ownership/lease/quota
ledger lives in an Azure Database for PostgreSQL flexible server, started with
`-ledger-driver=postgres -ledger-dsn=postgres://<user>:<password>@<server>.postgres.database.azure.com:5432/revika?sslmode=require`.
The Bicep template creates the VNet with a Container Apps subnet and a subnet delegated to
the flexible server, plus the `privatelink.postgres.database.azure.com` private DNS zone, so
the database is reachable only from the VNet. The admin password is generated by azd on first
provision; override the login with `azd env set POSTGRES_ADMIN_USER <name>`.
...
while the database traffic stays on the provisioned VNet. `/data` is the container's ephemeral
disk and holds only shard blobs and the libp2p identity, both re-created on restart. The node
is pinned to one replica because the ledger is per-node state.
