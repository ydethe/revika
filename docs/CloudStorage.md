...
Specification, API reference, and implementation guidelines for **`daemon`** and its
...
`daemon` is the **User-side background service**: it presents the user's encrypted,
erasure-coded, network-resident namespace as an ordinary-looking local filesystem, with
on-demand hydration ("files on demand"). It is the counterpart to `node` (the
headless blob server) and may optionally run a node in-process.
...
- **Persistence caveat (daemon TODO):** the current map is in-memory, so IDs are stable only
  ...
  seed for it) alongside the mount state — see §6.4. A future intrinsic `manifest.Entry.ID` would remove this concern.
...
anchor per User. Persisting/publishing it is the only piece of the mount stack
...
or C/GObject (Linux). Keep it *thin*: no revika logic, only translation.
...
Suggested flags (mirror `node` conventions): `-data`, `-mount <path>`, `-sync <path>`,
`-bootstrap`, `-dht`, `-signkey`, `-key`, `-repair-interval`, `-metrics`, `-v`.

### 6.2 Sync engine (`internal/sync`, planned)

OneDrive-style folder mirror, built entirely on the `Provider` API + a
...
The CLI already ships a stepping-stone of this two-phase model over the namespace: `ls rvk:<path>` fetches **only directory blobs**
...
- The daemon is **User-side and trusted**; it holds the keys. Nodes remain dumb/untrusted — no
  binding or daemon change relaxes that. All chunking, encryption, and erasure
...
Defer, as elsewhere in revika: payment/incentive layers, global consensus, and Byzantine reputation
