...
and the current sequence, not file contents. Your signing pubkey is in
ws/keys/user.sign.pub (or printed by keygen) :

     go run ./cmd/revika-ctl ls -root guest -owner "$(cat ws/keys/user.sign.pub)"
...
retrieve the revoked subtree (its old shard IDs were reclaimed) :
     ...
     go run ./cmd/revika-ctl ls -root guest -owner "$(cat ws/keys/user.sign.pub)"
     go run ./cmd/revika-ctl cp -root shared-dir.root.json -key guest/keys/user.key -node /ip4/127.0.0.1/tcp/4001/p2p/xxxxxx rvk: .guest_revoked   # expected to fail
