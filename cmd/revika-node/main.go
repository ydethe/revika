...
// geolocator builds the position estimator the /admin dashboard uses, from the
...
func geolocator(mode string, log *slog.Logger) geoip.Locator {
	...
	case "", "off":
		return nil
	case "ip-api":
		log.Warn("geoip: /admin map enabled via ip-api.com — peer PUBLIC IPs will be sent to that third party for geolocation",
