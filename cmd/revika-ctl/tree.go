...
// resolveRootCap loads a tree's root cap from a plaintext root-cap file
// (-manifest) or a wrapped cap file (-cap, unwrapped with -key). Exactly one
// source must be given — the directory analogue of resolveManifest.
func resolveRootCap(manifestPath, capPath, keyPath string) (manifest.ReadCap, error) {
	switch {
	case manifestPath != "" && capPath != "":
		return manifest.ReadCap{}, fmt.Errorf("use either -manifest or -cap, not both")
	case manifestPath != "":
		data, err := os.ReadFile(manifestPath)
		if err != nil {
			return manifest.ReadCap{}, err
		}
		var c manifest.ReadCap
		if err := json.Unmarshal(data, &c); err != nil {
			return manifest.ReadCap{}, fmt.Errorf("parse root cap %s: %w", manifestPath, err)
		}
		return c, nil
	case capPath != "":
		if keyPath == "" {
			return manifest.ReadCap{}, fmt.Errorf("-cap requires -key <privkey>")
		}
		...
		return manifest.UnwrapCap(priv, pub, sealed)
	default:
		return manifest.ReadCap{}, fmt.Errorf("provide -manifest <path> or -cap <path> -key <privkey>")
	}
}
