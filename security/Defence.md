...
La colonne **État d'implémentation** n'est renseignée que pour les contrôles *Code*, après analyse de `internal/` et `cmd/` :

- **Implémenté** — la primitive est réalisée intégralement dans le code.
- **Partiel** — couverture incomplète ou déléguée ; voir les notes sous la table.
- `—` — non applicable (contrôle *Mise en œuvre*, hors périmètre du code).

La colonne **Fichier(s) source revika** pointe vers le(s) fichier(s) Go où la primitive est
implémentée — chacun porte un bloc de commentaires `Defence controls` reliant le code à cet
inventaire (voir aussi la table maîtresse P1–P27 de [frameworks.md](frameworks.md)). `—` pour
les contrôles *Mise en œuvre* (hors code) ; `go.mod` pour SR-11 (pinning d'approvisionnement,
hors source Go).
...
- État des contrôles *Code* : **22** Implémenté / **4** Partiel

| Framework | ID | Description | Nombre d'occurrences | Pertinence revika | État d'implémentation | Fichier(s) source revika |
| --- | --- | --- | --- | --- | --- | --- |
...
| NIST SP 800-53 Rev 5 | AC-3 | Access Enforcement | 138 | Code | Implémenté | `internal/net/server.go`<br>`internal/stripe/stripe.go` |
...
| NIST SP 800-53 Rev 5 | SI-7 | Software, Firmware, and Information Integrity | 130 | Code | Implémenté | `internal/store/disk.go`<br>`internal/repair/repair.go`<br>`internal/stripe/stripe.go` |
...
| NIST SP 800-53 Rev 5 | SC-7 | Boundary Protection | 94 | Code | Implémenté | `internal/net/defense.go` |
| NIST SP 800-53 Rev 5 | SC-36 | Distributed Processing and Storage | 91 | Code | Implémenté | `internal/erasure/erasure.go`<br>`internal/net/placement.go` |
...
| MITRE D3FEND | D3-ITF | Inbound Traffic Filtering | 74 | Code | Implémenté | `internal/net/defense.go` |
...
| NIST SP 800-53 Rev 5 | SC-28 | Protection of Information at Rest | 51 | Code | Implémenté | `internal/crypto/crypto.go`<br>`internal/pipeline/pipeline.go` |
| NIST SP 800-53 Rev 5 | SC-4 | Information in Shared System Resources | 51 | Code | Implémenté | `internal/chunk/chunk.go` |
...
| NIST SP 800-53 Rev 5 | SC-5 | Denial-of-service Protection | 41 | Code | Implémenté | `internal/cap/pow.go`<br>`internal/net/defense.go` |
...
| NIST SP 800-53 Rev 5 | CP-10 | System Recovery and Reconstitution | 35 | Code | Implémenté | `internal/repair/repair.go` |
| NIST SP 800-53 Rev 5 | SC-6 | Resource Availability | 33 | Code | Implémenté | `internal/ledger/ledger.go` |
| MITRE D3FEND | D3-FH | File Hashing | 32 | Code | Implémenté | `internal/store/store.go`<br>`internal/store/disk.go` |
| MITRE D3FEND | D3-MAN | Message Authentication | 32 | Code | Implémenté | `internal/cap/signing.go`<br>`internal/stripe/stripe.go` |
...
| MITRE D3FEND | D3-MENCR | Message Encryption | 29 | Code | Implémenté | `internal/crypto/crypto.go`<br>`internal/cap/cap.go` |
| NIST SP 800-53 Rev 5 | IA-5 | Authenticator Management | 24 | Code | Implémenté | `internal/cap/signing.go` |
| NIST SP 800-53 Rev 5 | SC-8 | Transmission Confidentiality and Integrity | 22 | Code | Partiel | `internal/net/host.go` |
...
| MITRE D3FEND | D3-NTF | Network Traffic Filtering | 18 | Code | Implémenté | `internal/net/defense.go` |
| NIST SP 800-53 Rev 5 | SC-23 | Session Authenticity | 18 | Code | Implémenté | `internal/net/auth.go`<br>`internal/net/client.go` |
| NIST SP 800-53 Rev 5 | AU-10 | Non-repudiation | 17 | Code | Implémenté | `internal/cap/signing.go` |
| NIST SP 800-53 Rev 5 | SA-8 | Security and Privacy Engineering Principles | 17 | Code | Implémenté | `internal/net/server.go`<br>`internal/pipeline/pipeline.go` |
| NIST SP 800-53 Rev 5 | AU-9 | Protection of Audit Information | 16 | Code | Partiel | `internal/ledger/ledger.go` |
| NIST SP 800-53 Rev 5 | CP-2 | Contingency Plan | 15 | Mise en œuvre | — | — |
| NIST SP 800-53 Rev 5 | SC-12 | Cryptographic Key Establishment and Management | 15 | Code | Implémenté | `internal/cap/cap.go`<br>`internal/net/host.go` |
...
| NIST SP 800-53 Rev 5 | SI-10 | Information Input Validation | 7 | Code | Implémenté | `internal/net/proto.go` |
...
| NIST SP 800-53 Rev 5 | AU-6 | Audit Record Review, Analysis, and Reporting | 5 | Code | Partiel | `internal/net/metrics.go`<br>`internal/net/gcstats.go` |
...
| NIST SP 800-53 Rev 5 | SC-13 | Cryptographic Protection | 2 | Code | Implémenté | `internal/crypto/crypto.go` |
| NIST SP 800-53 Rev 5 | SR-11 | Component Authenticity | 2 | Code | Implémenté | `go.mod` |
| NIST SP 800-53 Rev 5 | SR-4 | Provenance | 2 | Code | Partiel | `internal/stripe/stripe.go` |
...
## Notes sur les contrôles *Partiel*

- **SC-8 — Transmission Confidentiality and Integrity** : le transport libp2p chiffre et
  authentifie par défaut (Noise/TLS), mais `internal/net/host.go` ne fixe pas explicitement
  d'option `libp2p.Security(...)` — la confidentialité en transit dépend des valeurs par défaut
  de la bibliothèque plutôt que d'un choix épinglé. Le point de métriques (`cmd/revika-node`)
  est du HTTP simple, TLS délégué à un reverse-proxy.
- **AU-9 — Protection of Audit Information** : le ledger SQLite (`internal/ledger`) impose une
  intégrité référentielle et se réconcilie avec le disque (`Reconcile`/`recomputeAccounts`),
  mais les enregistrements ne sont pas eux-mêmes signés ni append-only inviolables — un
  opérateur ayant accès à la base pourrait altérer des lignes de propriété sans détection.
  L'intégrité est structurelle, pas cryptographique.
- **AU-6 — Audit Record Review, Analysis, and Reporting** : métriques Prometheus, stats GC et
  logs structurés `slog` sont exposés (`internal/net/metrics.go`, `gcstats.go`) pour revue
  externe, mais aucune analyse, corrélation ou alerte n'est réalisée dans le code — la revue est
  laissée à l'outillage externe.
- **SR-4 — Provenance** : la propriété/origine par shard est tracée et chaque revendication est
  adossée à un jeton/grant signé au moment du stockage (`internal/net/server.go`,
  `internal/stripe`), mais le ledger ne conserve que le(s) pubkey(s) owner courant(s), pas une
  chaîne d'audit signée des jetons d'origine — la provenance est état-courant, pas historique.
